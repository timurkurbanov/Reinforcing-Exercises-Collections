...
room_201 = new_space["data"],["rooms"],0,["capacity"]
print(room_201)
...
def check():
    room = new_space['data']['rooms'][0]
    for event in new_space["data"]["events"]:
        if room['id'] == event['room_id']:
            if event['attendees'] <= room['capacity']:
                print(f"{event['id']}: will feet")
        # else:
        #     print(f"{event['id']}: will not feet") 
check()
